import { PlanSegment, DeckTrack } from '../types';
...
export async function mix(tracks: DeckTrack[], clipS: number): Promise<MixResult> {
  const res = await fetch(`${BASE_URL}/mix`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ tracks, clip_s: clipS }),
  });

  if (!res.ok) {
    let msg = `Mix failed (${res.status})`;
    try {
      const body = await res.json();
      if (body.error) msg = body.error;
    } catch { /* ignore */ }
